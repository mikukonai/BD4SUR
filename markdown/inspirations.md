...
☆先进信息系统：这个问题主要受信息论和热力学定律支配。主要的课题包括：高效、泛在、可靠的信息编码和通信，如5G、6G、量子传态通信等；高密度信息存储；高性能计算，如超算、量子计算等；智能控制和决策，也就是所谓的人工智能，如感知智能（自然语言、视觉、语音等）、认知智能和知识工程、大规模分布式计算、最优化控制和规划（博弈、调度、社会系统管理、精密机械、过程控制等）；复杂系统和仿生系统，如虚拟现实和数字孪生、社会科学、脑科学、生物信息学、人工生命等。问题的难点在于“先进”，是一个技术要求的集合，包括高容量、高效率、高精度、高可靠、泛在、实时（全时空覆盖）等。

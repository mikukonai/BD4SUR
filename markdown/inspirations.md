...
感觉我的语言能力很大程度上是建立在表情包的基础之上的。
另外，我感觉，表情包的运用水平，实际上也可以体现出表达能力。比如情报素养（理解亚文化、追踪流行语的能力），比如信息检索能力（从大量表情中寻找到合适的表情，重度依赖图形记忆能力），比如对多模态信息的综合理解能力（看图说话能力），比如说想象和抽象思维的能力（表情包恰恰是抽象的，因为表情包本质上是符号）。
表情包至少有两个作用，一是对话题的扰动和解构；二是将语义抽象化和符号化，提供充分的想象空间；三是将不便于表达的情感或意图委婉化、模糊化、钝化，当然也有可能起到反作用。
